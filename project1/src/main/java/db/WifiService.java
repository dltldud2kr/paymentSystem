...
public class WifiService {
	...
	public List<WifiInfo> list(double lat , double lnt){
        ...
        try {
            ...
            String sql = " select *, "
            		+ "    truncate((6371*acos(cos(radians(?))*cos(radians(X좌표))*cos(radians(Y좌표) "
            		+ "    -radians(?))+sin(radians(?))*sin(radians(X좌표)))),4) "
            		+ "    as distance "
            ...
            while(rs.next()){
            	...
            	wi.setDistance(rs.getDouble("distance"));
    ...
    public void dbInsert(TbPublicWifiInfo tw){
        String url = "jdbc:mariadb://localhost:3306/project_db?autoReconnect=true";
        String dbUserId = "projectuser";
        String dbPassword = "zerobase";
